...
for kpi in kpihash:
    ...
    if individualImages:
        if kpiimage != "None" and kpiimage != "" and kpiimage != "plot":
            ...
            # add the context elements to the scene
            try:
                context=metrichash['context'].split("|")
            except:
                context=[]
            if len(context)>0:
                renderView1 = pvutils.addContentGeometry(readerDisplay,renderView1,context)
                pvutils.adjustCamera(kpiimage, renderView1, metrichash, kpiimageflip)
    ...
    if export2Blender:
        try:
            blenderContext=metrichash['context'].split("|")
        except:
            blenderContext=[]
        try:

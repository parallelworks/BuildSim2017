...
class Client():
    ...
    def __init__(self, email, password, two_factor_key=None):
        ...
        self.session.mount(self.api, HTTPAdapter(max_retries=5))
    ...
    def documents(self,search):
        ...
        req = self.session.get(self.api + '/documents?q='+search+'&sortColumn=name&sortOrder=asc')
        req.raise_for_status()
        data = json.loads(req.text)
        return data
    ...
    def document(self,did):
        try:
            ...
            self.__auth()
        req = self.session.get(self.api + '/document/'+did)
        req.raise_for_status()
        data = json.loads(req.text)
        return data

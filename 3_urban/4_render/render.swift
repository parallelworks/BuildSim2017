...
app (file camPath) camPathCalc (file sceneOptions, file[] utils, file[] x3dFiles, int N) {
    bash "utils/runBlender.sh" "utils/camPathCalc.py" filename(sceneOptions) filename(camPath) N;
...
app (file out) render (file sceneOptions, int sceneNumber, file camPath, file[] utils, file[] x3dFiles, file grassFile) {
    bash "utils/runBlender.sh" "utils/renderScene.py" filename(sceneOptions) filename(out) sceneNumber filename(camPath);
}
...
app (file out) renderLoop (file sceneOptions, int sceneNumber, int Nloop, file camPath, file[] utils, file[] x3dFiles, file grassFile) {
    bash "utils/runBlender.sh" "utils/renderLoop.py" filename(sceneOptions) filename(out) sceneNumber Nloop filename(camPath);
}

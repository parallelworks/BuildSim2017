...
def createStreamTracer(metrichash, data_reader, data_display, isIndivImages):
    ...
    if (len(metrichash['position'].split()) == 4):
        Points = [float(x) for x in metrichash['position'].split()]
        print Points
        streamTracer.SeedType = 'Point Source'
        streamTracer.SeedType.Center = [Points[0],Points[1],Points[2]]
        streamTracer.SeedType.Radius = Points[3]
        streamTracer.SeedType.NumberOfPoints = int(metrichash['resolution'])
    elif (len(metrichash['position'].split()) == 6):
...
def addContentGeometry(readerDisplay, renderView1, context):
    if context != None and len(context) > 0:
        for i in context:
            ii = i.split(";")[0]
            r,g,b,a = [float(x) for x in i.split(";")[1].split(",")]
            dataReader = readDataFile(ii, None)
            sDisplay = Show(dataReader, renderView1)
            sDisplay.ColorArrayName = [None, '']
            sDisplay.SetRepresentationType('Surface')
            sDisplay.DiffuseColor = [r/255.0, g/255.0, b/255.0]
            sDisplay.Specular = 0
            sDisplay.Opacity = a
    return renderView1

...
mer=$(echo $loc | cut -d "_" -f3 | cut -d " " -f3)
viewtype=$8
...
# create the sky file
skyfunc="!gensky $month $day $starttime $sky -a $lat -o $lon -m $mer"
cat <<EOF> sky.rad
# Sky Definition
${skyfunc}
...
if [[ "$falsecolor" == "true" ]];then
    csh ./model/falsecolor2.csh -i rad.pic -s $fc_max -n 10 -m 1 -l Whm-2 > rad_fc.pic
